...
# RISC_V. The P&R flow is based on the EDI 13.1 official workshop and use the
...
set init_verilog "../syn/results/RISC_V.mapped.v"
...
# starting from the left offset 20.0 (roughly middle of the RISC_V module)
...
saveDesign db/RISC_V_powerplan.enc
...
saveDesign db/RISC_V_postroute.enc
...
verifyGeometry -report reports/RISC_V.geometry.rpt
verifyConnectivity -type all -report reports/RISC_V.connectivity.rpt
...
report_area -out_file reports/RISC_V.routed.area.rpt

# Power report
report_power -outfile reports/RISC_V.routed.power.rpt

# ------------------------------------------------------------------------------
# Step 7b. Exports the design
# ------------------------------------------------------------------------------
# SPEF file: parastic data of wires
rcOut -spef results/RISC_V.spef -rc_corner rc_worst

# SDF file: delay files for gates and wires
write_sdf results/RISC_V.sdf

# *.v: verilog netlist file
saveNetlist results/RISC_V.routed.v

# *.gds: GDS file
streamOut results/RISC_V.gds -mapFile streamOut.map \
