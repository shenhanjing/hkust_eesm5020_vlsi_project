...
analyze -format verilog {REG_ID.v REG_EX.v REG_WB.v alu_32bit.v alu_control.v ImmGen.v regfile.v control.v RISC_V.v }


elaborate RISC_V; # top module name

# store the unmapped results
write -hierarchy -format ddc -output results/RISC_V.unmapped.ddc
...
check_design > reports/RISC_V.check_design.rpt; # dump to the file
...
# Write out design
write -format verilog -hierarchy -output results/RISC_V.mapped.v
write -format ddc -hierarchy -output results/RISC_V.mapped.ddc
write_sdf results/RISC_V.mapped.sdf
write_sdc -nosplit results/RISC_V.mapped.sdc

# Generate reports
report_qor > reports/RISC_V.mapped.qor.rpt
report_timing -transition_time -nets -attribute -nosplit \
  > reports/RISC_V.mapped.timing.rpt
report_area -nosplit > reports/RISC_V.mapped.area.rpt
report_power -nosplit > reports/RISC_V.mapped.power.rpt
